/**
 * Uploads Unique's new photo package into Sanity and points each field at it.
 * Content-only change — no code deploy. Run once:
 *   node --use-system-ca scripts/apply-new-photos.mjs
 *
 * Re-runnable safely: it uploads fresh assets and re-points the fields.
 */
...
/** Build an image object with a reference + alt text. */
...
async function main() {
  console.log("Uploading 7 images…");
  ...
  console.log("Updating homepage photos…");
  await client
    .patch("homePage")
    .set({
  ...
  console.log("Updating the Believe Gathering event photo…");
  await client
    .patch("event-believe-gathering")
    .set({
      image: img(believe, "The Believe Gathering — a warm, candlelit Christmas banquet in navy and gold, with 'You are seen, you are loved, you belong here' banners"),
    })
    .commit();

  console.log("Setting the logo…");
  await client
    .patch("siteSettings")
    .set({ logo: img(logo, "Anchored Pathways Foundation") })
    .commit();

  console.log("\nDone. The live site will show the new photos within ~60 seconds.");
}
